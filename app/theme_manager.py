...
Обе темы порождаются из одного шаблона, чтобы не расходились при правках.
Бирюза #4ecdc4 — акцент, коралловый #ff6b6b — опасные действия, отступы
4/8/12/16/24/32.

QSS — не CSS: нет переменных, вложенности, calc(), transition и box-shadow,
...
def _arrow_url(theme, name='down'):
    """Путь к стрелке для QSS: только прямые слэши, иначе правило не
    применится."""
    from . import paths
...
/* Кнопки на карточке: заметные, но легче основных. */
#CardButton {{
    background: transparent;
    color: {text_muted};
...
/* Кнопки только с иконкой, размер 35x35: общий отступ 8px 16px оставил бы
   внутри один пиксель ширины. */
...
/* Без стрелки список неотличим от поля ввода. */
QComboBox::drop-down {{
...
/* Без стрелок кнопки счётчика выглядят серой полоской. */
